...
17. **NOT A BUG, but fixed as UX 2026-08-16** (Priscilla 15.08: "on return time you can select a later return, but not an earlier return / same with the pickup time, but opposite, you can't select a later pickup. We need both"): investigated, NO defect in the slot logic. Two independent proofs. (a) `unitsFreeForWindow` is monotone under window shrinkage - a shorter window's overlap set and its sampled instants are subsets of the longer one's, so peak can only fall. Consequence: on a MULTI-DAY range the in-hours pickup list is always upward-closed (a later pickup is always still offered) and the return list always downward-closed (an earlier return is always still offered). Her description is the exact inverse, so it cannot come from the availability filter on a range. (b) Exhaustive replay of the real BikeDetail pipeline against LIVE `/api/availability` data, 6 models x 40 days x 6 window lengths x 12 start-time pairs = 14383 states, running both auto-correct `useEffect`s to a fixed point: **0 slots that snap back**, i.e. every offered option is genuinely selectable. What she saw is the SAME-DAY case (one calendar tap = `effectiveRange` from==to), where the return list correctly starts after the pickup and the pickup list correctly ends where no free return follows - e.g. live 20.08 scooter-50: three units out all day, the fourth leaves 09:30, so the only same-day window is 09:00->09:30; live 17.08 bike-390: only pickup 18:30 with return 19:00. Both restrictions traced to real bookings. FIX SHIPPED (presentation only): the two dropdowns now render the WHOLE grid and grey out what can't be had as `HH:MM · <slotUnavailable>` (`pickupOptionGrid` / `returnOptionGrid` in BikeDetail.tsx) instead of hiding it, so a short list reads "not available" rather than "broken". The label is deliberately NEUTRAL and a NEW key (`calendar.slotUnavailable`, added to all 11 dictionaries): reusing `legendBooked` was tried and rejected by the cross-check, because a slot also drops out when the instant is free but the whole WINDOW clashes later (1 unit, foreign booking 21.08 09:00-12:00, range 20.-21.08: `isWindowFree(21.08 13:00->19:00)` is true, yet 13:00 would have read "Booked" while the calendar paints 21.08 half-green `legendPartial` - the page contradicting itself). INVARIANTS (check on any change here): (a) each grid must stay a SUPERSET of `pickupSlots`/`returnSlots`, else the selected value has no `<option>`; (b) a slot already PAST in Zadar wall-clock must be HIDDEN, never greyed - it is not unavailable, it is gone. Both grids therefore carry the same `zagrebNow` past filter with an `|| <selectable>.includes(s)` escape that keeps (a) true. The return grid needs its filter BEFORE the `!pickupTime` early return: on a day with no pickup left, `pickupTime` is "" and the start-before-end rule never runs, which is how "19:30 · ..." appeared at 20:00 the same evening (regression caught by the cross-check, fixed). Re-verified over 14400 states incl. simulated Zagreb clocks 08:00/12:00/18:31/20:00/21:45 and 3820 states with no pickup slot at all: 0 superset violations, 0 past slots greyed, 0 selected values without an option. Selectable set unchanged - only `pickupSlots`/`returnSlots` are enabled.

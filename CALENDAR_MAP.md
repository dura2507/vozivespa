...
13. **OPEN (product decision)**: (a) `fulfillment/route.ts` pickup/return acts on one bike id; forcing group-wide would break legitimate partial returns, so it is left per-bike (the dashboard FulfillButton already loops all group ids for "return the whole walk-in"). (b) Customer self-cancel (`booking/[token]/decision`) cancels one row of a group and emails nobody; whether the cancel link should drop the whole group or one bike is the owner's call. Neither is a silent bug; both await Thomas's intent.

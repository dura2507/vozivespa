...
2. **FIXED 2026-07-13**: Ghost/backup asymmetry. `findFreeUnit`, `findFreeUnits` and the three hint functions now skip bookings pinned to a unit outside the current pool (backup/ghost when `!includeBackup`, or inactive), so a ghost-parked rental no longer counts against regular K. Verified live on scooter-50: freeUnits 0 -> 1 for the 13.07 afternoon window where 1 regular bike was actually free, and stayed 0 in the genuinely-full morning window (no over-booking).

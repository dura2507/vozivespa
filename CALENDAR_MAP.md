...
2. **Ghost/backup asymmetry (CONFIRMED, open)**: `findFreeUnit`/`findFreeUnits` count bookings by `bike_id` only, with no `is_backup` filter, so a booking parked on the hidden reserve unit still counts against the regular K. Every display frees the regular unit when a booking is ghost-parked. Result: near capacity, the calendar advertises a day free but the submit/confirm gate rejects it, contradicting the ghost-swap premise (app/api/admin/bookings/[id]/group/route.ts:207+). Direction: false-reject.

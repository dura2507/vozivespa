...
- [x] **Aufpreis-Add-on jetzt DIREKT im Zeit-Dropdown** (Commit 9a333ec, ersetzt die alten Checkboxen aus a3f9e40): die Uhrzeit-Auswahl bietet früh 07:00-08:30 und spät 19:30-22:00 direkt als Optionen an, je mit "(+30€)"-Label. Auswahl = flat 30€ pro Seite. `billableDays()` clamp't Außer-Zeiten auf Öffnungszeiten -> KEIN Extra-Tag, nur der Aufschlag (no-op für alle In-Zeiten). Default snapt immer auf eine In-Zeit (Aufschlag ist immer bewusste Wahl). Echte gewählte Zeit wird gespeichert (Kalender/Bestätigung zeigen sie), Buchungs-Routen akzeptieren die erweiterten Slots (`isBookable*`). Shared-Model in `lib/pricing.ts`. Live gegen Produktion geprüft (Einzel: 90->120->150€; Gruppe: 90->120->150€).
  - **Gruppen-Server-Fix:** die `/group`-Route rechnet Preis serverseitig neu -> jetzt re-derived sie den Aufschlag aus den Zeiten und bäckt ihn in die gespeicherte Gruppen-Summe ein. Vorher war der Gruppen-Aufschlag nur kosmetisch und die 20%-Anzahlung/Online-Checkout (die `total_price_cents` summieren) haben ihn verpasst.
